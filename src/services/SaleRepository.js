...
export default class SaleRepository {
    ...
    async getSalesByName(name) {
        try {
            const sales = await this.saleDAO.getOneSalesByName(name);
            return sales;
        } catch (error) {
            throw error;
        }
    ...
    //                 totalAmount: { $sum: { $size: "$itemList" } } // o suma de pagos si prefieres
    //             }
    //         },
    //         { $sort: { _id: 1 } }
    //     ]);
    ...
    try{
        const monthlyTotals = await this.saleDAO.getSalesGroupedByDate(year);
